"""Application settings loaded from environment variables (Pydantic Settings v2).

No ``env_file`` here: the project ``.env`` is loaded into ``os.environ`` up-front
by ``app.infrastructure.config.load_configuration()`` (a walk-up search that is
CWD-independent, so it resolves the root-level ``.env`` even when running from
``api/``). In Docker the vars arrive as real environment variables via compose
``env_file``. Settings therefore read ``os.environ`` only — one source-agnostic path.
"""
