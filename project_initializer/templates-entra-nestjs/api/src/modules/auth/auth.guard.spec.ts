...
describe('EntraAuthGuard', () => {
  ...
  it('when getUser throws ForbiddenException, canActivate propagates it as 403', async () => {
    mockReflector.getAllAndOverride.mockReturnValue(false);
    mockAuthService.getUser.mockRejectedValue(new ForbiddenException('Insufficient scope'));
    const { ctx } = buildContext({ authorization: 'Bearer valid-token-bad-scope' });
