...
// Satisfy ConfigModule env validation without a live database or Entra connection.
process.env.DATABASE_URL = 'postgresql://localhost:5432/test';
process.env.DIRECT_URL = 'postgresql://localhost:5432/test';
process.env.ENTRA_TENANT_ID = 'test-tenant-id-e2e';
process.env.ENTRA_API_CLIENT_ID = 'test-api-client-id-e2e';
process.env.ENTRA_API_AUDIENCE = 'api://test-api-client-id-e2e';
process.env.ENTRA_API_SCOPE = 'access_as_user';
...
// Override AuthService so the guard does not make live JWKS network calls.
...
describe('AppModule entra (e2e)', () => {
  ...
  // Protected route — EntraAuthGuard calls AuthService.getUser which is mocked to succeed
  it('when GET /api/v1/test/ping is called with a Bearer token, returns 200 with { message: "pong" }', () => {
    return request(app.getHttpServer())
      .get('/api/v1/test/ping')
      .set('Authorization', 'Bearer valid-entra-jwt')
      .expect(200)
      .expect((res) => {
