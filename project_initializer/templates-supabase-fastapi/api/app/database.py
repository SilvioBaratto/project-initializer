"""
Synchronous database management for FastAPI with Supabase PostgreSQL.

This module provides:
- Supabase-optimized SQLAlchemy engine with psycopg2 driver
- Session Mode Pooler connection for persistent backend (port 5432)
- SSL enforcement (required by Supabase)
- Health check functionality with timeout protection
- Session management utilities
- Error handling and automatic recovery
- Production-ready pool configuration for 1000+ concurrent users
"""
...
class DatabaseManager:
    """
    Database manager for synchronous SQLAlchemy operations with Supabase.

    Features:
    - Supabase Session Mode Pooler connection (port 5432)
    - SSL enforcement (required by Supabase)
    - Connection pooling with automatic recovery
    - Prepared statement support (Session Pooler allows this)
    - Health checking with timeout protection
    - Thread-safe session management
    - Optimized for persistent backend (FastAPI on VM/container)
    """
    ...
    def initialize(self) -> None:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.info("Supabase database initialization successful")
    ...
    def _create_engine(self) -> None:
        ...
        # Build connection arguments for Supabase
        connect_args = self._build_supabase_connect_args()

        # Configure engine with Supabase-optimized settings
        engine_kwargs = {
            "url": settings.database_url,
            "poolclass": QueuePool,  # Persistent pool for persistent backend
    ...
    def _build_supabase_connect_args(self) -> Dict[str, Any]:
        """
        Build psycopg2-specific connection arguments for Supabase.

        Supabase requires:
        - SSL connection (sslmode=require)
        - Session Mode Pooler (port 5432) for persistent backends
        - Keep-alive settings to prevent idle connection drops
        - Prepared statements are supported (Session Pooler allows them)
        """
        connect_args = {
        ...
        # Add keep-alive settings to prevent pooler from closing idle connections
        connect_args.update(
            {
                "keepalives": 1,
                "keepalives_idle": 30,  # Start keepalives after 30s idle
                "keepalives_interval": 10,  # Keepalive interval
                "keepalives_count": 3,  # Max keepalive attempts
            }
        )
    ...
    def get_detailed_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "pooler_mode": "Session Mode (port 5432)",
            "ssl_enabled": True,
            "healthy": False,
            ...
            "configuration": {
                ...
                "backend_type": "persistent (FastAPI on VM/container)",
            },
        }
    ...
    def create_all_tables(self) -> None:
        ...
        Note: In production with Supabase, use Alembic migrations instead.
...
def init_db() -> None:
    ...
    Note: create_all_tables() is NOT called here for Supabase.
    ...
    try:
        ...
        # DO NOT create tables with Supabase - use Alembic migrations instead
